...
| 注意事项                                                     |
| ------------------------------------------------------------ |
| 你可以在没有做过2D的情况下继续学习这个系列。然而，如果你是游戏开发的新手，我们建议你从2D开始。3D游戏代码总是比较复杂的，而2D系列会给你打下基础，让你更轻松地跟上。 |

我们准备了一些游戏资产，所以我们可以直接跳到代码。你可以在这里下载它们：
...
我们将首先为玩家的动作制作一个基本原型。然后我们将添加在屏幕周围随机生成的怪物。之后，我们将实现跳跃和压扁机制，然后用一些漂亮的动画来完善游戏。我们将以分数和重玩界面作为总结。
...
在这第一部分，我们将设置游戏区域。让我们开始吧，导入起始资产并设置游戏场景。

我们已经准备了一个 Godot 项目，其中包含我们将用于本教程的 3D 模型和声音，链接在索引页面中。如果您还没有这样做，您可以在此处下载存档：略

下载后，在您的计算机上解压缩 .zip 存档。打开 Godot 项目管理器并单击导入按钮。

![](images/Snipaste_2022-10-06_09-36-05.png)

在导入弹出窗口中，输入刚创建的目录squash_the_creeps_start/的完整路径。你可以点击右边的浏览按钮，打开一个文件浏览器，导航到该文件夹包含的project.godot文件。

![](images/Snipaste_2022-10-06_09-36-57.png)

点击导入和编辑，在编辑器中打开项目。

![](images/Snipaste_2022-10-06_09-37-46.png)

启动项目包含一个图标和两个文件夹：art/ 和 fonts/。在那里，您会找到我们将在游戏中使用的美术资源和音乐。

译者注：音乐资源我放到了Music中了。

![](images/Snipaste_2022-10-06_09-39-32.png)

有两个3D模型，player.glb和mob.glb，一些属于这些模型的材料，和一个音乐轨道。

##### 设置可玩区域

我们将使用普通节点作为其根来创建我们的主场景。在场景停靠栏中，单击左上角由“+”图标表示的添加节点按钮，然后双击节点。将节点命名为“Main”。或者，要将节点添加到场景中，您可以按 Ctrl + a（或 macOS 上的 Cmd + a）。

![](images/Snipaste_2022-10-06_09-42-12.png)

按Ctrl + s（macOS上为Cmd + s）将场景保存为Main.tscn。

我们将首先添加一个地板，以防止角色坠落。为了创建像地板、墙壁或天花板这样的静态碰撞体，你可以使用 StaticBody 节点。它们需要 CollisionShape 子节点来定义碰撞区域。选择 Main 节点，添加一个 StaticBody 节点，然后添加一个 CollisionShape 节点。将 StaticBody 重命名为Ground。

![](images/Snipaste_2022-10-06_09-46-55.png)

CollisionShape 旁边会出现一个警告标志，因为我们尚未定义它的形状。如果单击该图标，则会出现一个弹出窗口，为您提供更多信息。

![](images/Snipaste_2022-10-06_09-47-31.png)

要创建一个形状，在选择了CollisionShape后，前往检查器并点击 Shape 属性旁边的[空]字段。创建一个新的Box Shape。

![](images/Snipaste_2022-10-06_09-48-35.png)

盒子形状非常适合平坦的地面和墙壁。它的厚度使其能够可靠地阻挡快速移动的物体。

一个盒子的线框出现在视口中，带有三个橙色点。您可以单击并拖动它们，以交互方式编辑形状的范围。我们还可以在检查器中精确设置大小。单击 BoxShape 以展开资源。在 X 轴上将其范围设置为 30，在 Y 轴上设置为 1，在 Z 轴上设置为 30。

![](images/Snipaste_2022-10-06_09-52-26.png)

| 注意事项                                                     |
| ------------------------------------------------------------ |
| 在 3D 中，平移和大小单位以米为单位。盒子的总尺寸是其范围的两倍：地平面上 60 x 60 米，高 2 个单位。地平面由 X 和 Z 轴定义，而 Y 轴代表高度。 |

碰撞形状是不可见的。我们需要添加一个与之配套的可视化地面。选择Ground节点并添加一个MeshInstance作为其子节点。

![](images/Snipaste_2022-10-06_09-55-05.png)

在检查器中，点击 Mesh 旁边的字段，创建一个CubeMesh资源来创建一个可见的立方体

![](images/Snipaste_2022-10-06_09-56-21.png)

再一次，默认情况下它太小了。单击立方体图标以展开资源并将其大小设置为 60、2 和 60。由于立方体资源使用大小而不是范围，我们需要使用这些值，使其与我们的碰撞形状相匹配。

![](images/Snipaste_2022-10-06_09-58-06.png)

您应该会在视口中看到一个覆盖网格以及蓝色和红色轴的宽灰色板。

![](images/Snipaste_2022-10-06_09-59-26.png)

| 注意事项                                                     |
| ------------------------------------------------------------ |
| 如果您看不到上图中的 3D 对象操纵器，请确保视图上方的工具栏中的选择模式处于激活状态。 |

![](images/Snipaste_2022-10-06_10-01-16.png)

将Ground向下移动 1 米。视口左下角的标签告诉您正在平移多少节点。

![](images/Snipaste_2022-10-06_10-02-56.png)

| 注意事项                                                     |
| ------------------------------------------------------------ |
| 向下移动Ground节点会使两个子节点一起移动。确保你移动的是地面节点，而不是MeshInstance或者CollisionShape。 |

让我们添加一个定向光，这样我们的场景就不会全是灰色的。选择 Main 节点并添加一个 <font color = "red">DirectionalLight</font> 作为它的子节点。我们需要移动它并旋转它。通过单击并拖动操纵器的绿色箭头将其向上移动，然后单击并拖动红色弧以使其围绕 X 轴旋转，直到地面被点亮。(地板变白)

在检查器中，通过点击复选框打开Shadow->启用。

![](images/Snipaste_2022-10-06_11-25-55.png)

此时，您的项目应该如下所示。

![](images/Snipaste_2022-10-06_11-27-50.png)

这就是我们的起点。在下一部分中，我们将致力于玩家场景和基地的移动。

#### 玩家场景和输入动作

在接下来的两节课中，我们将设计玩家场景、注册自定义输入动作以及编写玩家移动代码。到最后，您将拥有一个可在八个方向移动的可玩角色。

通过进入左上方的场景菜单并点击新建场景来创建一个新的场景。创建一个 KinematicBody 节点作为根节点并命名为Player。

| 另见                                                     |
| -------------------------------------------------------- |
| 要了解有关不同物理节点类型的更多信息，请参阅物理学介绍。 |

现在，我们将为角色的 3D 模型创建一个基本装备。这将允许我们稍后在播放动画时通过代码旋转模型。

添加一个 Spatial 节点作为 Player 的子节点，并将其命名为 Pivot。然后，在 文件系统停靠区中，通过双击展开 art/ 文件夹并将 player.glb 拖放到 Pivot 节点上。

![](images/Snipaste_2022-10-06_11-36-56.png)

| 注意事项                                                     |
| ------------------------------------------------------------ |
| .glb文件包含基于开源GLTF 2.0规范的3D场景数据。它们是一种现代的、强大的 替代FBX等专有格式的文件，Godot也支持这种格式。为了制作这些文件，我们在Blender 3D中设计了模型，并将其导出为GLTF。 |

与所有类型的物理节点一样，我们需要一个碰撞形状让我们的角色与环境发生碰撞。再次选择 Player 节点并添加一个 CollisionShape。在检查器中，将 SphereShape 分配给 Shape 属性。球体的线框出现在角色下方。

![](images/Snipaste_2022-10-06_11-40-25.png)

这将是物理引擎用来与环境碰撞的形状，因此我们希望它更适合 3D 模型。通过拖动视口中的橙色点将其缩小一点。我的球体半径约为 0.8 米。

然后，向上移动形状，使其底部大致与网格平面对齐。

![](images/Snipaste_2022-10-06_20-16-46.png)

![](images/Snipaste_2022-10-06_20-17-54.png)

您可以通过单击 Character 或 Pivot 节点旁边的眼睛图标来切换模型的可见性。

![](images/Snipaste_2022-10-06_20-19-01.png)

将场景另存为 Player.tscn。

准备好节点后，我们几乎可以开始写代码了。但首先，我们需要定义一些输入动作。

##### 创建输入动作

为了移动角色，我们将听从玩家的输入，比如按下方向键。在Godot中，虽然我们可以用代码写出所有的键绑定，但有一个强大的系统允许你为一组键和按钮分配一个标签。这简化了我们的脚本，使其更易读。

这个系统是输入映射。要访问其编辑器，请前往项目菜单并选择项目设置...。

![](images/Snipaste_2022-10-07_05-45-16.png)

在顶部，有多个选项卡。单击输入映射。此窗口允许您在顶部添加新操作；它们是你的标签。在底部，您可以将键绑定到这些操作。

![](images/Snipaste_2022-10-07_05-46-20.png)

Godot项目有一些为用户界面设计的预定义动作，我们可以在这里使用。但我们要定义自己的动作来支持游戏手柄。

我们将为我们的动作命名为 move_left、move_right、move_forward、move_back和 jump。

![](images/Snipaste_2022-10-07_05-48-50.png)

创建这五个动作。您的窗口应该把它们都列在底部。

![](images/Snipaste_2022-10-07_05-49-44.png)

要将键或按钮绑定到操作，请单击其右侧的“+”按钮。对 move_left 执行此操作，然后在下拉菜单中单击 Key。

![](images/Snipaste_2022-10-07_05-50-52.png)

此选项允许您添加键盘输入。出现一个弹出窗口，等待您按下某个键。按向左箭头键，然后单击确定。

![](images/Snipaste_2022-10-07_05-51-33.png)

用 A 键执行相同操作。

![](images/Snipaste_2022-10-07_05-52-40.png)

现在让我们添加对游戏手柄左操纵杆的支持。再次单击“+”按钮，但这一次，选择 Joy Axis。

![](images/Snipaste_2022-10-07_06-01-30.png)

弹出窗口为您提供两个下拉菜单。在左侧，您可以按索引选择游戏手柄。设备 0 对应于第一个插入的游戏手柄，设备 1 对应于第二个，依此类推。您可以在右侧选择要绑定到输入操作的操纵杆和方向。保留默认值并按添加按钮。

![](images/Snipaste_2022-10-07_06-02-27.png)

对其他输入操作执行相同操作。例如，将右箭头 D 和左操纵杆的右轴绑定到 move_right。绑定所有键后，您的界面应如下所示。

![](images/Snipaste_2022-10-07_06-05-10.png)

我们还有跳跃动作要设置。绑定空格键和游戏手柄的A键。要绑定游戏手柄的按钮，在菜单中选择Joy Button选项。

![](images/Snipaste_2022-10-07_06-06-05.png)

保留默认值并单击添加按钮。

![](images/Snipaste_2022-10-07_06-06-53.png)

你的跳跃输入动作应该是这样的。

![](images/Snipaste_2022-10-07_06-07-50.png)

这就是我们在这个游戏中需要的所有动作。你可以使用这个菜单来标记你的项目中的任何一组按键和按钮。

在下一部分中，我们将编写代码并测试玩家的移动。

#### 使用代码移动玩家

是时候编写代码了！我们将使用我们在上一部分中创建的输入动作来移动角色。

右键单击播放器节点，选择附加脚本，为其添加一个新的脚本。在弹出的窗口中，在按下创建按钮之前，将模板设置为空。

![](images/Snipaste_2022-10-07_06-09-53.png)
...
这些是运动物体的常见属性。速度是结合了速度(速率)和方向的 3D 矢量。在这里，我们将它定义为一个属性，因为我们想要跨帧更新和重用它的值。

| 注意事项                                                     |
| ------------------------------------------------------------ |
| 这些值与二维码完全不同，因为距离以米为单位。在 2D 中，一千个单位（像素）可能只对应于屏幕宽度的一半，而在 3D 中，它是一公里。 |

现在让我们对运动进行编码。我们首先在 _physics_process() 中使用全局 Input 对象计算输入方向向量。
...
在这里，我们将使用 _physics_process() 虚函数进行所有计算。与 _process() 一样，它允许您每帧更新节点，但它是专门为与物理相关的代码设计的，例如移动运动学或刚体。
